...
namespace IranPost.Net.PersianDateTime
{
    ...
    /// </summary>
    /// <see cref="http://www.codeproject.com/Articles/13664/Farsi-Library-Working-with-Dates-Calendars-and-Dat"/>
    internal class PersianCalendar : Calendar
    {
        /// <summary>
        ...
        )
        {
            ...
            {
                throw new InvalidPersianDateException("Invalid date time format.");
            }
        ...
        private static bool HasLeapFrac(
            int year
        )
        {
        ...
        )
        {
            ...
            catch (ArgumentOutOfRangeException)
            {
                ...
                throw new InvalidPersianDateException("Invalid month day in date time.");
            }
        ...
        )
        {
            if (year != 0)
            {
                ...
                catch (ArgumentOutOfRangeException)
                {
                    ...
                    throw new InvalidPersianDateException("Invalid year encountered.");
                }
            }
        ...
        /// <remarks>In the Persian calendar leap years are applied every 4 or 5 years according to a certain pattern that iterates in a 2820-year cycle. A common year has 365 days and a leap year has 366 days.</remarks>
        public int NumberOfLeapYearsUntil(
            int year
        )
        {
        ...
        private int NumberOfLeapYearsUntil(
            bool validate,
            int year
        )
        {
        ...
        )
        {
            if (validate)
            {
                if (era < 0 || 1 < era)
                {
                    throw new InvalidPersianDateException("Invalid era encountered.");
                }
            }
        ...
        )
        {
            if (validate)
            {
                ...
                if (day < 1 || maxday < day)
                {
                    if (day == 30 && month == 12)
                    {
                        throw new InvalidPersianDateException("Invalid leap year in date time.");
                    }
                    else
                    {
        ...
        private void GetYearAndRemainingDays(
            bool validate,
            DateTime time,
            out int year,
            out int days
